...
public class NewChatGui extends AbstractGui {
    ...
    /** Duplicate stacking state: the last message text, how often it repeated and how many lines it drew. */
    ...
    private void setChatLine(ITextComponent chatComponent, int chatLineId, int updateCounter, boolean displayOnly) {
        ...
        if (!displayOnly && chatLineId == 0 && net.slate.module.impl.ChatModule.stacksDuplicates()) {
            String raw = chatComponent.getString();
            if (raw.equals(this.lastRawMessage) && this.lastDrawnCount > 0
                    && this.drawnChatLines.size() >= this.lastDrawnCount) {
                for (int k = 0; k < this.lastDrawnCount; ++k) {
                ...
                chatComponent = chatComponent.deepCopy().appendSibling(
                        new net.minecraft.util.text.StringTextComponent(" (" + this.lastRepeatCount + ")")
                                .applyTextStyle(net.minecraft.util.text.TextFormatting.GRAY));
            } else {
        ...
        if (!displayOnly && chatLineId == 0) {
            this.lastDrawnCount = list.size();
        }
        int historyLimit = net.slate.module.impl.ChatModule.historySize();
        while (this.drawnChatLines.size() > historyLimit) {
        ...
        if (!displayOnly) {
            ...
            while (this.chatLines.size() > historyLimit) {
