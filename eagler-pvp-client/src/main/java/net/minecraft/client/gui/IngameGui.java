...
public class IngameGui extends AbstractGui {
   ...
   public void renderGameOverlay(float partialTicks) {
      ...
      if (!net.slate.module.impl.EffectsModule.replacesVanilla()) {
         this.renderPotionEffects();
      }
      net.slate.Slate.renderHud(partialTicks);
   ...
   private void renderAttackIndicator() {
      ...
      if (net.slate.module.impl.FreelookModule.hidesCrosshair()
            || this.mc.currentScreen instanceof net.slate.ui.ClickGuiScreen) {
         return;
      }
      if (gamesettings.thirdPersonView == 0) {
         if (this.mc.playerController.getCurrentGameType() != GameType.SPECTATOR || this.func_212913_a(this.mc.objectMouseOver)) {
            ...
            if (gamesettings.showDebugInfo && !gamesettings.hideGUI && !this.mc.player.hasReducedDebug() && !gamesettings.reducedDebugInfo) {
               ...
               if (net.slate.module.impl.CrosshairModule.renderCrosshair(this.scaledWidth, this.scaledHeight)) {
                  net.slate.module.impl.HitMarkerModule.render(this.scaledWidth, this.scaledHeight);
                  GlStateManager.enableBlend();
                  GlStateManager.color4f(1.0F, 1.0F, 1.0F, 1.0F);
                  this.mc.getTextureManager().bindTexture(GUI_ICONS_LOCATION);
               } else {
   ...
   private int drawSingleplayerStats() {
      ...
      int i = 0;
      // Server timing is debug information: keep it on the F3 screen so it never overlaps the HUD.
      if (this.mc.gameSettings.showDebugInfo && SingleplayerServerController.isWorldRunning()) {
   ...
   private void renderScoreboard(ScoreObjective objective) {
      ...
      boolean hideNumbers = net.slate.module.impl.CleanScoreboardModule.hideNumbers();
      boolean hideBackground = net.slate.module.impl.CleanScoreboardModule.hideBackground();
      ...
      for(Score score : collection) {
         ...
         String s1 = ScorePlayerTeam.formatMemberName(scoreplayerteam, new StringTextComponent(score.getPlayerName())).getFormattedText();
         if (!hideNumbers) {
            s1 = s1 + ": " + TextFormatting.RED + score.getScorePoints();
         }
