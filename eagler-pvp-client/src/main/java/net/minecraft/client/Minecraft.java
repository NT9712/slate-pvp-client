...
public class Minecraft extends RecursiveEventLoop<Runnable> implements ISnooperInfo, IWindowEventListener, AutoCloseable {
    ...
    public void updateDisplay(boolean limitFramerate) {
        ...
        int limit = limitFramerate && this.isFramerateLimited() ? this.getFramerateLimit() : 0;
        this.mainWindow.update(this.gameSettings.fullscreen, limit);
        this.profiler.endSection();
    }
    ...
    private int getFramerateLimit() {
        int slate = net.slate.module.impl.PowerSaverModule.frameLimit();
        if (slate > 0) {
            return slate;
        }
        ...
        if (this.leftClickCounter <= 0) {
            if (this.objectMouseOver == null) {
                ...
                switch (this.objectMouseOver.getType()) {
                    case ENTITY:
                        net.slate.Slate.onAttack(((EntityRayTraceResult) this.objectMouseOver).getEntity());
