...
public class ParticleManager implements IFutureReloadListener {
   ...
   /** Cached live particle count, refreshed once per tick by the client particle limiter. */
   private int liveParticleCount;

   public void addEffect(Particle effect) {
      if (net.slate.module.impl.ParticlesModule.cutExplosions()
            && (effect instanceof HugeExplosionParticle || effect instanceof LargeExplosionParticle)) {
         return;
      }
      int limit = net.slate.module.impl.ParticlesModule.maxParticles();
      if (limit >= 0 && this.queue.size() + this.liveParticleCount >= limit) {
         return;
      }
   ...
   private void refreshParticleCount() {
      int n = 0;
      for (Queue<Particle> q : this.byType.values()) {
         if (q != null) {
            n += q.size();
         }
      }
      this.liveParticleCount = n;
   }
