...
public class GameRenderer implements AutoCloseable, IResourceManagerReloadListener {
   ...
   /** Whether dynamic FOV (sprint boost) is disabled by a module. */
   private boolean disableDynamicFOV = false;
   ...
   public void tick() {
      ...
      // If dynamic FOV is disabled, force fovModifierHand to 1.0 every tick
      if (this.disableDynamicFOV) {
   ...
   /**
    * Called by NoDynamicFOVModule to disable/enable dynamic FOV (sprint boost).
    * TeaVM-safe: no reflection needed.
    */
   public void setDisableDynamicFOV(boolean disable) {
      this.disableDynamicFOV = disable;
      if (disable) {
         this.fovModifierHand = 1.0F;
         this.fovModifierHandPrev = 1.0F;
      }
   ...
   private double getFOVModifier(ActiveRenderInfo p_215311_1_, float p_215311_2_, boolean p_215311_3_) {
      if (this.debugView) {
         ...
         d0 *= net.slate.module.impl.ZoomModule.fovScale(p_215311_2_);
         return d0;
      }
   }
   ...
   private void hurtCameraEffect(float partialTicks) {
      if (this.mc.getRenderViewEntity() instanceof LivingEntity) {
         ...
         f = MathHelper.sin(f * f * f * f * (float)Math.PI) * net.slate.module.impl.HurtCamModule.scale();
         if (f == 0.0F) {
            return;
         }
